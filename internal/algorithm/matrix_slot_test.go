...
func TestScheduleMatrixPlaceBlockUpdatesClassAndTeacher(t *testing.T) {
	...
	matrix := NewScheduleMatrix([]uint{classID}, []uint{teacherID}, []MatrixBlock{block}, nil)
...
func TestScheduleMatrixRejectsTeacherConflict(t *testing.T) {
	...
	matrix := NewScheduleMatrix([]uint{classA, classB}, []uint{teacherID}, []MatrixBlock{blockA, blockB}, nil)
...
func TestScheduleMatrixUsesSlotIndexNotSlicePosition(t *testing.T) {
	...
	matrix := NewScheduleMatrix([]uint{classID}, nil, []MatrixBlock{block}, daySlots)
...
func matrixTestBlock(classID uint, teacherID *uint, duration int) MatrixBlock {
	return MatrixBlock{
		ID:        uuid.New(),
		TeacherID: teacherID,
		SubjectID: 1,
