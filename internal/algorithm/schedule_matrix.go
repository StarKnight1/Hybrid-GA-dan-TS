...
type ScheduleMatrix struct {
	...
	pjokSubjectID            uint
	slotEndByDay             map[string]map[int]string // precomputed day → slotIndex → endTime
}
...
// SetPJOKSubjectID enables the PJOK 2JP deadline hard constraint on this matrix.
// When set, any 2JP block for this subject must end by pjokDeadlineEndTime.
func (s *ScheduleMatrix) SetPJOKSubjectID(id uint) {
	s.pjokSubjectID = id
}
...
func NewScheduleMatrix(classes []uint, teachers []uint, blocks []MatrixBlock, daySlots DaySlots) *ScheduleMatrix {
	...
	slotEndByDay := make(map[string]map[int]string, len(s.daySlots))
	for d, slots := range s.daySlots {
		m := make(map[int]string, len(slots))
		for _, sl := range slots {
			m[sl.Index] = sl.EndTime
		}
		slotEndByDay[d] = m
	}
	s.slotEndByDay = slotEndByDay
...
func (s *ScheduleMatrix) canPlace(block MatrixBlock, day string, startSlot int) error {
	...
	// PJOK 2JP hard deadline: the block's last slot must end by pjokDeadlineEndTime.
	if s.pjokSubjectID != 0 && block.SubjectID == s.pjokSubjectID && block.Duration == 2 {
		lastIdx := startSlot + block.Duration - 1
		if endTime := s.slotEndByDay[day][lastIdx]; endTime > pjokDeadlineEndTime {
			return fmt.Errorf("PJOK 2JP block %s must end by %s (ends at %s)", block.ID, pjokDeadlineEndTime, endTime)
		}
	}
