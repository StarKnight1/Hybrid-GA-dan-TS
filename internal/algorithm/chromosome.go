...
func BuildCandidateIndex(blocks []MatrixBlock, pjokSubjectID uint, daySlots DaySlots) map[uuid.UUID][]Gene {
	if daySlots == nil {
...
func SmartChromosome(blocks []MatrixBlock, candidateIndex map[uuid.UUID][]Gene, groups GroupIndex, daySlots DaySlots, pjokSubjectID uint, rng *rand.Rand) Chromosome {
	type item struct {
		origIdx  int
		block    MatrixBlock
	...
	if pjokSubjectID != 0 {
		...
		matrix.SetPJOKSubjectID(pjokSubjectID)
	}
...
// ConstraintAwareCrossover builds a child chromosome by trying, for each block,
// the gene from parent a first, then parent b, then a random valid candidate.
// A temporary matrix is maintained during construction so each chosen gene is
// checked against already-placed blocks — this reduces unplaced counts in
// the child compared to blind uniform crossover.
// Blocks sharing a GroupKey are treated as a single unit: all members get the
// same gene, and the placement check applies to every member.
func ConstraintAwareCrossover(
	...
	for _, it := range items {
		...
		switch {
		case caxCanPlace(matrix, members, blocks, geneA):
			chosen = geneA
		case caxCanPlace(matrix, members, blocks, geneB):
			chosen = geneB
		default:
			candidates := candidateIndex[block.ID]
			for _, pi := range rng.Perm(len(candidates)) {
				if caxCanPlace(matrix, members, blocks, candidates[pi]) {
...
// caxCanPlace returns true if gene g can be placed for every member block in matrix.
func caxCanPlace(matrix *ScheduleMatrix, members []int, blocks []MatrixBlock, g Gene) bool {
	if !g.IsPlaced() {
		return false
	}
	for _, j := range members {
